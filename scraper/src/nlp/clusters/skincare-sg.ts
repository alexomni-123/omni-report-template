...
export const skincareSgStageMatchers = {
  problem:
    /\b(acne|pimple|breakout(ing)?|cystic|comedone|whitehead|blackhead|zit|sensitive|reactive|irritat|stinging|burning|allergic|itchy|rash|red(ness)?|flush(ing)?|sweat(y)?|humid(ity)?|oily|greasy|shiny|t[-\s]?zone|hyperpigmentation|melasma|dark\s*spot|sun\s*spot|scar(ring|s)?|pih|pie|uneven|patchy|blotchy|sunburn(t|ed)?|sun\s*damage|tan(ned)?|peeling|dry|dehydrated|tingl(e|ing)|burn(ed|ing|t)?|flare(d|up)?|hate|hated|terrible|awful|nightmare|wasted|gave\s*up|cant\s*find|broke\s*out|broke\s*me\s*out|reaction|reacted|white\s*cast|sticky|greasy|patchy\s*finish|never\s*work(ed|s)?|made\s*it\s*worse|getting\s*worse|nothing\s*helps|cant\s*sleep|painful|hurt(s|ing)?|tight(ness)?|flake?(s|y|ing)?|cracking|bleed(ing)?|infected|inflamed|inflammation|swollen|swelling|cyst(s)?|pus|popping|picking|squeez(e|ing|ed))\b/i,
  brand:
